...
def has_cadex_conf():
    ...
    valid_cadex_conf = False

    # ensure cadex_conf exists & readable
    #
    # if not, print a message stating that Cadex Core must be installed and
    # configured, including JSONRPC access in cadex.conf
    try:
        f = io.open(config.cadex_conf)
        valid_cadex_conf = True
    except IOError as e:
        print(e)

    return valid_cadex_conf
...
def main():
    ...
    if not has_cadex_conf():
        print("CadexCore must be installed and configured, including JSONRPC access in cadex.conf")
        sys.exit(1)
