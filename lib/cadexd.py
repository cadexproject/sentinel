...
class CadexDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some cadexd methods
    ...
    @classmethod
    def from_cadex_conf(self, cadex_dot_conf):
        from cadex_config import CadexConfig
        config_text = CadexConfig.slurp_config_file(cadex_dot_conf)
        creds = CadexConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = cadexlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = cadexlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = cadexlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_cadexd_proto_version_with_sentinel_ping)
