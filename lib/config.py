...
sentinel_cfg = CadexConfig.tokenize(sentinel_config_file)
sentinel_version = "1.3.0"
min_cadexd_proto_version_with_sentinel_ping = 70207


def get_cadex_conf():
    if sys.platform == 'win32':
        cadex_conf = os.path.join(os.getenv('APPDATA'), "CADEXCOIN/cadex.conf")
    else:
        home = os.environ.get('HOME')

        cadex_conf = os.path.join(home, ".CADEXCOIN/cadex.conf")
        if sys.platform == 'darwin':
            cadex_conf = os.path.join(home, "Library/Application Support/CADEXCOIN/cadex.conf")

    cadex_conf = sentinel_cfg.get('cadex_conf', cadex_conf)
