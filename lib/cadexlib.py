...
def is_valid_cadex_address(address, network='mainnet'):
    ...
    cadex_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Cadex address
        return False

    if (address_version != cadex_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('cadex.conf')
