...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, cadexd):
        golist = cadexd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_cadexd(cadexd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_cadexd(self, cadexd, rec):
        import decimal
        import cadexlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from cadexd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from cadexd, with every run
        ...
        # get/create, then sync payment amounts, etc. from cadexd - Cadexd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(cadexd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from cadexd! %s" % e)
            govobj.vote_delete(cadexd)
            return (govobj, None)
    ...
    def vote_delete(self, cadexd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(cadexd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, cadexd, signal, outcome):
        import cadexlib
        ...
        output = cadexd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = cadexlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(cadexd, signal)

    def sync_network_vote(self, cadexd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = cadexd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = CADEXD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 cadex addr, non-multisig
            if not cadexlib.is_valid_cadex_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Cadex address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Cadex Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (cadexlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = CADEXD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not cadexlib.is_valid_cadex_address(addr, config.network):
    ...
    def hash(self):
        import cadexlib
        return cadexlib.hashit(self.serialise())
